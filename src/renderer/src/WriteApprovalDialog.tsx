...
export function WriteApprovalDialog({ request, resolving, onResolve }: WriteApprovalDialogProps) {
  ...
  useEffect(() => {
    ...
    const closeOnEscape = (event: KeyboardEvent) => {
      if (event.key === "Escape" && !resolving) onResolve(false)
    }
  ...
  return (
    <div className="fixed inset-0 z-[70] grid place-items-center bg-black/70 p-6 backdrop-blur-sm">
      <button type="button" aria-label="Cancel database write" disabled={resolving} onClick={() => onResolve(false)} className="absolute inset-0 cursor-default focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-danger focus-visible:outline-none disabled:cursor-wait" />
      <section role="alertdialog" aria-modal="true" aria-labelledby="write-approval-title" aria-describedby="write-approval-description" aria-busy={resolving} className="relative z-10 w-full max-w-lg rounded-lg border border-line-strong bg-panel p-5 shadow-2xl">
